...
public class DashboardService {
  ...
  public DashboardResponse getDashboard(User user) {
    ...
    Map<Integer, UserMastery> masteryBySkillId =
        masteries.stream()
            .collect(Collectors.toMap(mastery -> mastery.getSkill().getId(), Function.identity()));

    Map<Integer, List<Exercise>> exercisesBySkillId = new HashMap<>();

    List<DashboardSkillMasteryResponse> skillMastery =
        masteries.stream().map(this::mapSkillMastery).toList();

    List<DashboardAttemptResponse> recentAttempts =
        attemptRepository.findLatestForEachExerciseByUser(user).stream()
            .map(this::mapAttempt)
            .toList();
    ...
    List<DashboardPathProgressResponse> pathProgress =
        learningPathRepository.findAll().stream()
            ...
            .map(
                path ->
                    mapPathProgress(
                        path, masteryBySkillId, completedExerciseIds, exercisesBySkillId))
            .toList();

    Exercise recommendedExercise = recommendationService.recommendNextExercise(user);
  ...
  private DashboardUserResponse mapUser(User user) {
    return new DashboardUserResponse(
        user.getUsername(), user.getEmail(), toInstant(user.getCreatedAt()));
  }

  private DashboardOverviewResponse buildOverview(
      User user, List<UserMastery> masteries, Set<Integer> completedExerciseIds) {
  ...
  private DashboardSkillMasteryResponse mapSkillMastery(UserMastery mastery) {
  ...
  private DashboardExerciseSummaryResponse mapExercise(Exercise exercise) {
    return new DashboardExerciseSummaryResponse(
        exercise.getId(),
        exercise.getTitle(),
        exercise.getDescription(),
        exercise.getDifficulty(),
        exercise.getSkills().stream().map(Skill::getName).toList());
  }
  ...
  private DashboardPathProgressResponse mapPathProgress(
    ...
    List<DashboardPathSkillResponse> skills =
        path.getSkills().stream()
            .map(
                skill ->
                    mapPathSkill(skill, masteryBySkillId, completedExerciseIds, exercisesBySkillId))
            .toList();

    Set<Integer> pathExerciseIds =
        path.getSkills().stream()
            .flatMap(skill -> getExercisesForSkill(skill, exercisesBySkillId).stream())
            .map(Exercise::getId)
            .collect(Collectors.toSet());

    int completedExercises =
        (int) pathExerciseIds.stream().filter(completedExerciseIds::contains).count();
  ...
  private DashboardPathSkillResponse mapPathSkill(
    ...
    List<Exercise> exercises = getExercisesForSkill(skill, exercisesBySkillId);

    int completedExercises =
        (int)
            exercises.stream().map(Exercise::getId).filter(completedExerciseIds::contains).count();
  ...
  private List<Exercise> getExercisesForSkill(
      Skill skill, Map<Integer, List<Exercise>> exercisesBySkillId) {
    return exercisesBySkillId.computeIfAbsent(
        skill.getId(), ignored -> exerciseRepository.findBySkillsContainingOrderByIdAsc(skill));
  }
  ...
  private BigDecimal calculateAverageMastery(List<UserMastery> masteries) {
    ...
    return total.divide(BigDecimal.valueOf(masteries.size()), 2, RoundingMode.HALF_UP);
  }
